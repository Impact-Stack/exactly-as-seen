  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled ? "bg-background/95 backdrop-blur border-b border-border shadow-sm" : "bg-background/85 backdrop-blur border-b border-border/60"
      }`}
    >
      <div className="container-narrow flex items-center justify-between h-[72px]">
        ...
        <nav className="hidden lg:flex items-center gap-8">
          {navLinks.map((link) =>
            link.hasDropdown ? (
              ...
              >
                <button className="flex items-center gap-1 text-sm text-foreground/80 link-hover font-semibold">
                  {link.label}
                  <ChevronDown className="w-3.5 h-3.5" />
                </button>
                {activeMenu === link.label && (
                  <div className="absolute top-full left-1/2 -translate-x-1/2 pt-2">
                    <div className="glass rounded-xl shadow-xl p-8 min-w-[620px] bg-card">
                      <div className={`grid gap-8 ${megaMenus[link.label].columns.length === 2 ? "grid-cols-2" : "grid-cols-3"}`}>
                        {megaMenus[link.label].columns.map((col) => (
                          <div key={col.title}>
                            <p className="tag-label text-primary/90 mb-3">{col.title}</p>
                            <ul className="space-y-2">
                              {col.links.map((l) => (
                                <li key={l.label}>
                                  <Link to={l.href} className="text-sm text-foreground/80 link-hover block py-1">
                                    {l.label}
                                  </Link>
                                </li>
                              ))}
              ...
              <Link key={link.label} to={link.href!} className="text-sm text-foreground/80 link-hover font-semibold">
                {link.label}
              </Link>
            )
          )}
        ...
        <div className="hidden lg:flex items-center gap-4">
          <Link
            to="/contact"
            className="bg-primary text-primary-foreground text-label uppercase px-6 py-3 rounded-lg hover:bg-primary-dark transition-all duration-300"
          >
            Get Started
          </Link>
        </div>
          ...
          <nav className="flex flex-col gap-4 pt-4">
            {navLinks.map((link) => (
              <Link
                key={link.label}
                to={link.href || "/"}
                className="text-body text-foreground font-semibold py-2"
