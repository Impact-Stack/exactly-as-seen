...
export default function InvestSwipeSection() {
  ...
  return (
    <section className="section-padding bg-[#0D0D0D] border-y border-white/10 overflow-hidden" ref={ref}>
      <div className="container-narrow">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <motion.div initial={{ opacity: 0, x: -30 }} animate={isInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.5 }}>
            <span className="inline-block bg-green-500/10 text-green-400 text-label px-3 py-1.5 rounded-lg mb-6 border border-green-500/30">
              FINTECH PRODUCT | TARGET Q3 2026
            </span>
            <h2 className="text-section font-display mb-2 text-white">InvestSwipe</h2>
            <p className="text-card-title text-slate-400 font-normal mb-6">Accessible investing for emerging markets.</p>
            <p className="text-body text-slate-400 mb-8 max-w-[500px]">
            ...
            <ul className="space-y-3 mb-8">
              {["R10 starting investment", "Short-form education content", "Simple swipe interaction model", "Community learning features"].map((f) => (
                <li key={f} className="flex items-center gap-3 text-body text-slate-300">
                  <span className="w-1.5 h-1.5 bg-blue-500 rounded-full" /> {f}
                </li>
              ))}
            </ul>
            <Link to="/investswipe" className="inline-block bg-blue-500 text-white px-8 py-4 rounded-lg text-body font-semibold hover:bg-blue-400 transition-colors">
              Join Waitlist
            </Link>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, x: 30 }} animate={isInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.5, delay: 0.1 }} className="flex justify-center">
            <div className="relative">
              <div className="absolute inset-0 bg-blue-500/15 blur-[70px] rounded-full" />
